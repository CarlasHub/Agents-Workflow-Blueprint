...
function previewMarkup(config, excludedFiles = new Set()) {
  ...
  const removedLabel = `${inactiveFiles.length} ${countLabel(inactiveFiles.length, 'removed file')}`;
  ...
  return `
    ...
    <div class="metrics-grid" aria-label="Generated workspace summary">
      <div aria-label="${escapeHtml(includedLabel)}">
        <strong>${activeFiles.length}</strong>
        <span>${countLabel(activeFiles.length, 'included file')}</span>
      </div>
      <div aria-label="${escapeHtml(governanceLabel)}">
        <strong>${governanceCount}</strong>
        <span>${countLabel(governanceCount, 'governance doc')}</span>
      </div>
      <div aria-label="${escapeHtml(qualityLabel)}">
        <strong>${qualityCount}</strong>
        <span>${countLabel(qualityCount, 'quality doc')}</span>
      </div>
      <div aria-label="${escapeHtml(removedLabel)}">
        <strong>${inactiveFiles.length}</strong>
        <span>${countLabel(inactiveFiles.length, 'removed file')}</span>
      </div>
    </div>
    ...
    <div class="file-browser live-file-browser" aria-label="Live generated project preview" aria-live="polite">
      ...
      <div class="terminal-feed">
        ...
        ${activeFiles.length ? '<p class="feed-more">Use Remove to exclude a file from the generated ZIP.</p>' : '<p class="feed-more is-warning">No files are currently included.</p>'}
      ...
      <form class="workbench-grid" id="project-form">
        <aside class="command-rail" aria-label="Generation workflow">
          ...
          <ol>
            ...
            <li><strong>04</strong><span>quality standard</span></li>
