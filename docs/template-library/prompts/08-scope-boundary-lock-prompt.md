...
25. Make out-of-scope items explicit enough that the agent cannot silently implement them.
26. Name adjacent work that is tempting but unsafe to include.
27. Require explicit evidence when claiming something was intentionally left unchanged.
28. Reject additions that improve polish while violating the approved boundary.
