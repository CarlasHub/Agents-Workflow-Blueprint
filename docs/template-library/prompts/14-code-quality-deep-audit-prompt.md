...
25. Inspect maintainability, not personal style preference.
26. Tie every critique to readability, testability, ownership, or defect risk.
27. Separate cleanup that is safe now from cleanup that needs a planned refactor.
28. Flag code that is hard to reason about even when it currently works.
