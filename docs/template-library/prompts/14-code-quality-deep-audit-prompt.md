...
26. Inspect maintainability, not personal style preference.
27. Tie every critique to readability, testability, ownership, or defect risk.
28. Separate cleanup that is safe now from cleanup that needs a planned refactor.
29. Flag code that is hard to reason about even when it currently works.
