...
| JavaScript units | `npm run test:unit` | Escaping, safe links, Markdown rendering, generated document routing, raw-source download boundaries, normalisation, filtering, manifest data, project names, generated files, and approval state. |
...
| Browser behaviour | `npm run test:e2e` | Homepage, analytics-consent, privacy, static-catalogue, rendered-guide, rendered-prompt, and builder workflows in Chromium, Firefox, and WebKit, including keyboard modal behaviour and extracted ZIP content. |
| Automated accessibility | `npm run test:a11y` | Axe checks across twelve representative page states in Chromium, Firefox, and WebKit plus DOM assertions. |
